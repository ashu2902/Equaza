...
export function SideBySideShowcase({ items, className }: SideBySideShowcaseProps) {
          ...
          {items.map((item, idx) => {
            const Figure = (
              <figure className="relative rounded-xl overflow-hidden border border-neutral-200">
                <div className="relative aspect-[4/3]">
                  {item.image?.src ? (
                    <SafeImage src={item.image.src} alt={item.image.alt || ''} fill className="object-cover" />
                  ) : (
            ...
            return (
              <div key={`${item.title}-${idx}`}>
                {item.href ? (
                  <Link href={item.href} className="block hover:opacity-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#98342d]">
                    {Figure}
                  </Link>
                ) : (
                  Figure
