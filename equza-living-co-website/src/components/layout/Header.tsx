...
export function Header() {
  ...
  const isAdminRoute =
    typeof nextPathname === 'string' && nextPathname.startsWith('/admin');
    ...
    <>
        ...
        <>
          ...
          <div className='fixed top-2 left-0 right-0 z-40 flex justify-center pointer-events-none'>
            <Link
              ...
              aria-label='Equza Living Co. Home'
            >
              <Image
                src='/equza_-_logo__3_-removebg-preview.svg'
                alt='Equza Living Co. logo'
                width={140}
                height={40}
                priority
              />
      ...
      <motion.header
        className={`fixed top-0 left-0 w-full z-50 transition-colors ${
          showHeader
            ? 'bg-[#D8BF9F] text-warm-900 shadow-sm'
            : 'bg-transparent text-warm-900 pointer-events-none'
        }`}
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex items-center justify-between h-16 lg:h-20'>
            ...
            <nav className='hidden lg:flex items-center space-x-8'>
              <Link
                href='/'
                className={`transition-colors font-medium ${showHeader ? 'text-warm-900 hover:text-warm-900/80' : 'text-warm-700 hover:text-primary-600'}`}
              >
                Home
              </Link>
              <Link
