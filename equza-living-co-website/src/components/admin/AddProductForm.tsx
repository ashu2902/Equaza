...
export function AddProductForm({
  ...
  const initialFormData: FormData = initial
    ? {
        ...
        dimensions:
          initial.specifications?.availableSizes?.[0]?.dimensions || '',
  ...
  const handleInputChange = (field: keyof FormData, value: any) => {
    ...
    if (field === 'name' && typeof value === 'string') {
      ...
      console.log(
        `🔗 AddProductForm: Auto-generated slug from "${value}":`,
        generatedSlug
      );
      setFormData((prev) => ({
        ...prev,
  ...
  const handleArrayChange = (
    ...
    console.log(
      `📋 AddProductForm: Array field "${field}" - ${checked ? 'Adding' : 'Removing'} "${value}"`
    );
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    console.log(
      `📸 AddProductForm: Starting image upload for ${files.length} files`
    );
    ...
    try {
      ...
      if (invalidFiles.length > 0) {
        ...
        alert(
          `Only image files are allowed. Skipped: ${invalidFiles.map((f) => f.name).join(', ')}`
        );
      }

      console.log(
        `📸 AddProductForm: Valid files for upload: ${validFiles.length}`
      );

      setFormData((prev) => {
        const newImages = [...prev.images, ...validFiles];
        console.log(
          `📸 AddProductForm: Total images after upload: ${newImages.length}`
        );
        return {
          ...prev,
          images: newImages,
  ...
  const removeImage = (index: number) => {
    ...
    console.log(
      `🗑️ AddProductForm: Removing new image at index ${index}:`,
      removedFile?.name
    );
    ...
    setFormData((prev) => {
      ...
      console.log(
        `🗑️ AddProductForm: Remaining new images: ${newImages.length}`
      );
      return {
        ...prev,
        images: newImages,
  ...
  const removeExistingImage = async (index: number) => {
    ...
    console.log(
      `🗑️ AddProductForm: Removing existing image:`,
      existingImage.storageRef
    );
    ...
    try {
      ...
      if (result.success) {
        setFormData((prev) => {
          ...
          console.log(
            `🗑️ AddProductForm: Remaining existing images: ${newExistingImages.length}`
          );
          return {
            ...prev,
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.description.trim()) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - Description is required'
      );
    }
    ...
    if (!formData.story.trim()) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - Product story is required'
      );
    }
    ...
    if (parsedMaterials.length === 0) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - No materials provided'
      );
    }
    ...
    if (!formData.weaveType.trim()) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - Weave type is required'
      );
    }
    ...
    if (!formData.dimensions.trim()) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - Dimensions are required'
      );
    }
    ...
    if (formData.collections.length === 0) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - No collections selected'
      );
    }
    ...
    if (!hasAnyImage) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - No images available (existing or new)'
      );
    }
    ...
    if (formData.priceVisible && formData.price <= 0) {
      ...
      console.log(
        '❌ AddProductForm: Validation failed - Price is required when visible'
      );
    }
    ...
    console.log(
      `🔍 AddProductForm: Form validation ${isValid ? 'PASSED' : 'FAILED'}`
    );
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    console.log(
      `🚀 AddProductForm: Starting form submission (isDraft: ${isDraft})`
    );
    ...
    startTransition(async () => {
      try {
        ...
        if (formData.images.length > 0) {
          console.log(
            `📸 AddProductForm: Uploading ${formData.images.length} images (CLIENT-SIDE)`
          );
          console.log(
            `📸 AddProductForm: Files to upload:`,
          ...
          try {
            ...
            const { uploadMultipleFiles } = await import(
              '@/lib/firebase/storage'
            );

            // Generate file paths on client side
            const timestamp = Date.now();
            const filePaths = formData.images.map(
              (file, index) =>
            ...
            uploadedImages = uploadUrls
              .map((url, index) => ({
                ...
                storageRef:
                  filePaths[index] || `products/${Date.now()}-${index}`,
            ...
            console.error(
              '❌ AddProductForm: Client-side image upload failed:',
              uploadError
            );
        ...
        const productData: Omit<Product, 'id' | 'createdAt' | 'updatedAt'> = {
          ...
          specifications: {
            ...
            availableSizes: [
              { dimensions: formData.dimensions, isCustom: false },
            ],
        ...
        console.log(
          `🔄 AddProductForm: Calling ${mode === 'edit' ? 'updateAdminProduct' : 'createAdminProduct'} server action`
        );
        ...
        if (result.success) {
          ...
          console.error(
            '❌ AddProductForm: Server action failed:',
            result.message
          );
          setSubmitError(result.message);
          if (result.errors) {
            console.error(
              '❌ AddProductForm: Validation errors from server:',
              result.errors
            );
        ...
        console.error(
          '❌ AddProductForm: Unexpected error during submission:',
          error
        );
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <SlideUp>
            <Card className='!bg-white !border-[#98342d]/20'>
              ...
              <CardContent className='space-y-4'>
                <div>
                  <Label htmlFor='name' className='text-[#98342d]/80'>
                    Product Name *
                  </Label>
                  <Input
                    id='name'
                ...
                <div>
                  <Label htmlFor='slug' className='text-[#98342d]/80'>
                    URL Slug *
                  </Label>
                  <Input
                    id='slug'
                ...
                <div>
                  <Label htmlFor='description' className='text-[#98342d]/80'>
                    Description *
                  </Label>
                  <Textarea
                    id='description'
                ...
                <div>
                  <Label htmlFor='story' className='text-[#98342d]/80'>
                    Product Story *
                  </Label>
                  <Textarea
                    id='story'
              ...
              <CardContent className='space-y-4'>
                <div className='flex items-center space-x-2 mb-4'>
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange('priceVisible', e.target.checked)
                    }
                    className='rounded border-[#98342d]/30 text-[#98342d] focus:ring-[#98342d]'
                  />
                  <Label htmlFor='priceVisible' className='text-[#98342d]/80'>
                    Show price publicly
                  </Label>
                </div>
                ...
                {formData.priceVisible && (
                  <Grid cols={2} gap='md'>
                    <div>
                      <Label htmlFor='price' className='text-[#98342d]/80'>
                        Starting Price *
                      </Label>
                      <Input
                        id='price'
                    ...
                    <div>
                      <Label htmlFor='currency' className='text-[#98342d]/80'>
                        Currency
                      </Label>
                      <select
                        id='currency'
          ...
          <SlideUp delay={0.2}>
            <Card className='!bg-white !border-[#98342d]/20'>
              ...
              <CardContent className='space-y-4'>
                ...
                <div>
                  <Label htmlFor='materialsText' className='text-[#98342d]/80'>
                    Materials *
                  </Label>
                  <Input
                    id='materialsText'
                    ...
                    onChange={(e) =>
                      handleInputChange('materialsText', e.target.value)
                    }
                ...
                <Grid cols={2} gap='md'>
                  <div>
                    <Label htmlFor='weaveType' className='text-[#98342d]/80'>
                      Weave Type *
                    </Label>
                    <Input
                      id='weaveType'
                      ...
                      onChange={(e) =>
                        handleInputChange('weaveType', e.target.value)
                      }
                  ...
                  <div>
                    <Label htmlFor='origin' className='text-[#98342d]/80'>
                      Origin
                    </Label>
                    <Input
                      id='origin'
                  ...
                  <div>
                    <Label htmlFor='dimensions' className='text-[#98342d]/80'>
                      Dimensions *
                    </Label>
                    <Input
                      id='dimensions'
                  ...
                  <div>
                    <Label htmlFor='craftTime' className='text-[#98342d]/80'>
                      Craft Time
                    </Label>
                    <Input
                      id='craftTime'
                      ...
                      onChange={(e) =>
                        handleInputChange('craftTime', e.target.value)
                      }
          ...
          <SlideUp delay={0.3}>
            <Card className='!bg-white !border-[#98342d]/20'>
              ...
              <CardContent className='space-y-4'>
                <div>
                  <Label htmlFor='seoTitle' className='text-[#98342d]/80'>
                    SEO Title
                  </Label>
                  <Input
                    id='seoTitle'
                ...
                <div>
                  <Label htmlFor='seoDescription' className='text-[#98342d]/80'>
                    SEO Description
                  </Label>
                  <Textarea
                    id='seoDescription'
              ...
              <CardContent className='space-y-4'>
                <div className='flex items-center space-x-2'>
                  <input
                    ...
                    className='rounded border-[#98342d]/30 text-[#98342d] focus:ring-[#98342d]'
                  />
                  <Label htmlFor='isActive' className='text-[#98342d]/80'>
                    Published
                  </Label>
                </div>
                ...
                <div className='flex items-center space-x-2'>
                  <input
                    ...
                    className='rounded border-[#98342d]/30 text-[#98342d] focus:ring-[#98342d]'
                  />
                  <Label htmlFor='isFeatured' className='text-[#98342d]/80'>
                    Featured Product
                  </Label>
                </div>
              </CardContent>
                ...
                <div className='space-y-4'>
                  ...
                  {mode === 'edit' && formData.existingImages.length > 0 && (
                    <div className='space-y-2'>
                      <Label className='text-[#98342d]/80'>
                      ...
                      <div className='grid grid-cols-2 gap-3'>
                        {formData.existingImages.map((img, index) => (
                          <div
                            key={index}
                            className='bg-[#98342d]/5 rounded-lg p-3'
                          >
                                ...
                                {img.url ? (
                                  <img
                                    ...
                                    onError={(e) => {
                                      ...
                                      parent.innerHTML =
                                        '<svg class="h-8 w-8 text-[#98342d]/50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>';
                                    }}
                                  />
                                ) : (
                      ...
                      <p className='text-xs text-[#98342d]/60'>
                        Removing an existing image will permanently delete it
                        from storage.
                      </p>
                    </div>
                  )}
                        ...
                        {formData.images.map((file, index) => (
                          ...
                          >
                            <div className='flex flex-col items-center space-y-2'>
                              ...
                              <div className='text-center'>
                                <p className='text-sm font-medium text-[#98342d] truncate'>
                                  {file.name}
                                </p>
                                <p className='text-xs text-[#98342d]/60'>
                  ...
                  <div className='space-y-2 mt-2'>
                    {styleCollections.map((collection) => (
                      ...
                      >
                        ...
                        />
                        <Label
                          htmlFor={`style-${collection.id}`}
                          className='text-[#98342d]/80 text-sm'
                        >
                          {collection.name}
                        </Label>
                      </div>
