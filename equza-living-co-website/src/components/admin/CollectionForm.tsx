...
export function CollectionForm({ mode, initial }: Props) {
  ...
  const [heroStorageRef, setHeroStorageRef] = useState(initial?.heroImage?.storageRef || '');
  ...
  async function removeHeroImage() {
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, heroImage: err?.message || 'Failed to remove image' }));
    }
  }
                ...
                <div className="mb-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
                  <div className="flex flex-col items-center space-y-2">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-medium text-gray-900">Hero Image</p>
                      <p className="text-xs text-gray-500">{heroAlt || 'No alt text'}</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
