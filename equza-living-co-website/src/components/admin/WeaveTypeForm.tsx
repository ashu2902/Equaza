...
} from '@/lib/actions/admin'; // Import from index
...
export function WeaveTypeForm({ mode, initial }: Props) {
  ...
  const [imageStorageRef, setImageStorageRef] = useState(
    initial?.image?.storageRef || ''
  );
  ...
  async function removeImage() {
    ...
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        image: err?.message || 'Failed to remove image',
      }));
    }
  }
