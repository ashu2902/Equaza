...
const filterSections: (
  collections: FilterOption[],
  materials: FilterOption[]
) => FilterSection[] = (collections, materials) => [
  {
    id: 'collectionId',
    label: 'Collections',
    type: 'select',
...
export const FilterSidebar: FC<FilterSidebarProps> = ({
        ...
        case 'select':
          if (sectionId === 'collectionId') {
            newFilters[sectionId] =
              newFilters[sectionId] === value ? undefined : value;
          }
          break;
        ...
        case 'multiselect':
          if (sectionId === 'materials') {
            ...
            if (currentMaterials.includes(value)) {
              newFilters.materials = currentMaterials.filter(
                (m) => m !== value
              );
            } else {
  ...
  const renderFilterSection = (section: FilterSection) => {
    ...
    return (
      <Card key={section.id} className='border border-stone-200'>
        ...
        >
          <Typography variant='body1' className='font-medium text-stone-900'>
            {section.label}
          </Typography>
          {isExpanded ? (
  ...
  const sidebarContent = (
    <div className='space-y-4'>
      ...
      {/* Filter Sections */}
      <div className='space-y-3'>{sections.map(renderFilterSection)}</div>
              ...
              return material ? (
                ...
                >
                  ...
                  <Button
                    ...
                    size='sm'
                    onClick={() =>
                      handleFilterChange('materials', materialId, 'multiselect')
                    }
                    className='h-auto p-1'
                  >
                    <X className='w-3 h-3' />
            ...
            {filters.isFeatured && (
              <div className='flex items-center justify-between'>
                ...
                <Button
                  ...
                  size='sm'
                  onClick={() =>
                    handleFilterChange('isFeatured', 'true', 'toggle')
                  }
                  className='h-auto p-1'
                >
                  <X className='w-3 h-3' />
  ...
  if (isMobile) {
    return <div className={`${className}`}>{sidebarContent}</div>;
  }

  return <div className={`sticky top-6 ${className}`}>{sidebarContent}</div>;
};
