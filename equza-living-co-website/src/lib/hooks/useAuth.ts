...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user: User | null) => {
      ...
      if (user) {
        try {
          console.log('🔍 useAuth: Checking admin status via JWT custom claims');
          
          // Check admin status using JWT custom claims (same as login flow)
          console.log('🔍 useAuth: Fetching ID token with claims for UID:', user.uid);
          const tokenResult = await user.getIdTokenResult();
          
          console.log('🎯 useAuth: Custom claims:', tokenResult.claims);
          console.log('👑 useAuth: Admin claim present:', !!tokenResult.claims.admin);
          ...
          if (isAdmin) {
            ...
            console.log('❌ useAuth: User does not have admin privileges in JWT claims');
          }
