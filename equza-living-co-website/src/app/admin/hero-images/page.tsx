...
export default function AdminHeroImagesPage() {
  ...
  const updateForm = useCallback(
    (pageType: PageType, field: keyof HeroImageForm, value: any) => {
      setForms((prev) => ({
        ...prev,
        [pageType]: {
          ...prev[pageType],
  ...
  const handleFileChange = useCallback(
    (pageType: PageType, file: File | null) => {
  ...
  const handleDragOver = useCallback(
    (pageType: PageType, e: React.DragEvent) => {
      e.preventDefault();
  ...
  const deleteImage = useCallback(
    async (pageType: PageType) => {
      if (
        !confirm(
          `Are you sure you want to remove the hero image for ${pageTypeLabels[pageType]}?`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        alert(
          `Failed to delete hero image: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      } finally {
  ...
  const handleSave = useCallback(
    async (pageType: PageType) => {
      ...
      try {
        ...
        if (form.file) {
          const uploadPath = generatePaths.adminUpload(
            form.file.name,
            'hero-images'
          );
        ...
        alert(
          `Failed to update hero image: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      } finally {
        setSaving((prev) => ({ ...prev, [pageType]: false }));
      }
        ...
        <Container>
          <div className='flex items-center justify-center h-64'>
            ...
            <span className='ml-2 text-red-600'>
              Error loading hero images: {error}
            </span>
          </div>
        </Container>
      </div>
      ...
      <div className='min-h-screen bg-gray-50 py-8'>
        <Container>
          <FadeIn>
            <div className='mb-8'>
              ...
              <Typography variant='body' className='text-gray-600 mb-6'>
                Manage hero background images for all pages. Upload new images
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
            {(Object.keys(pageTypeLabels) as PageType[]).map(
              (pageType, index) => {
                const heroImage = heroImages[pageType];
                const form = forms[pageType];
                        ...
                        <div className='mb-4'>
                          <div className='flex items-center justify-between'>
                            <div>
                              ...
                              >
                                {heroImage?.uploadedAt
                                  ? `Updated: ${new Date(heroImage.uploadedAt).toLocaleDateString()}`
                                  : 'No image set'}
                              </Typography>
                            </div>
                              ...
                              <div className='relative'>
                                ...
                                <div className='relative aspect-video bg-gray-100 rounded-lg overflow-hidden'>
                                  ...
                                  <div className='absolute bottom-2 left-2 right-2'>
                                    ...
                                    >
                                      File: {form.file.name} (
                                      {(form.file.size / 1024 / 1024).toFixed(
                                        2
                                      )}
                                      MB)
                                    </Typography>
                          ...
                          <div className='space-y-4'>
                            ...
                            <div className='flex space-x-3'>
                              <Button
                                ...
                                disabled={
                                  isSaving || !form.imageUrl || !form.altText
                                }
                                className='flex-1'
                              >
                                {isSaving ? (
